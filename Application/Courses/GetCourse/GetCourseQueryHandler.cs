...
namespace Application.Courses.GetCourse
{
    ...
    /// </remarks>
    public class GetCourseQueryHandler(AppCoursesDbContext context, IMapper mapper) : IRequestHandler<GetCourseQuery, Result<GetCourseResponse>>
    {
        ...
        public async Task<Result<GetCourseResponse>> Handle(GetCourseQuery request, CancellationToken cancellationToken)
        {
            var course = await _context.Courses!.Where(x => x.Id == request.Id)
                            ...
                            .Include(x => x.Photos)
                            .ProjectTo<GetCourseResponse>(_mapper.ConfigurationProvider)
                            .FirstOrDefaultAsync(cancellationToken: cancellationToken);

            return Result<GetCourseResponse>.Success(course!);
        }
    }
}